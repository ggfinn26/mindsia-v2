...
@section('title', 'Detail Marketing Bonus Rule')
@section('header_title', 'Marketing Bonus Rule')
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex items-start justify-between gap-4">
        <div>
            <a href="{{ route('bonus.marketing-rules.index') }}" class="mb-1 flex items-center gap-1 text-xs font-medium text-slate-500 hover:text-slate-700">
                <span class="material-symbols-outlined text-[16px]">arrow_back</span>
                Marketing Bonus Rules
            </a>
        ...
        <div class="flex gap-2">
            @can('bonus.marketing-rule.update')
                <form method="POST" action="{{ route('bonus.marketing-rules.toggle-active', $rule) }}">
                    @csrf @method('PATCH')
            ...
            @endcan
            @can('bonus.marketing-rule.delete')
                <form method="POST" action="{{ route('bonus.marketing-rules.destroy', $rule) }}" onsubmit="return confirm('Hapus rule ini beserta semua tiernya?')">
                    @csrf @method('DELETE')
                    <button type="submit" class="inline-flex min-h-9 items-center gap-1 border border-red-200 px-3 text-xs font-semibold text-red-700 hover:bg-red-50">Hapus</button>
                </form>
            @endcan
        </div>
    </div>
    ...
    <div class="border border-slate-200 bg-white">
        ...
        @can('bonus.marketing-rule.update')
        <form method="POST" action="{{ route('bonus.marketing-rules.update', $rule) }}" class="p-5">
            @csrf @method('PUT')
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                    <label class="mb-1 block text-xs font-semibold text-slate-700">Dasar bonus</label>
                    <select name="bonus_basis" required class="block w-full border-slate-300 text-sm focus:border-[#215aac] focus:ring-[#215aac]">
                        @foreach ($basisLabels as $val => $label)
                            <option value="{{ $val }}" @selected(old('bonus_basis', $rule->bonus_basis) === $val)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    <label class="mb-1 block text-xs font-semibold text-slate-700">Revenue basis</label>
                    <input type="text" name="revenue_basis" value="{{ old('revenue_basis', $rule->revenue_basis) }}" maxlength="100" class="block w-full border-slate-300 text-sm focus:border-[#215aac] focus:ring-[#215aac]">
                </div>
            </div>
            <div class="mt-4">
            ...
            <div class="mt-4">
                <button type="submit" class="inline-flex min-h-9 items-center gap-2 bg-[#215aac] px-4 text-sm font-semibold text-white hover:bg-[#194a91]">
                    Simpan perubahan
                </button>
            </div>
        </form>
        @else
        <div class="grid grid-cols-2 gap-4 p-5 sm:grid-cols-3 text-sm">
            <div><p class="text-xs text-slate-500">Scope</p><p class="font-medium text-slate-900">{{ $scopeLabels[$rule->scope_type] }}</p></div>
            <div><p class="text-xs text-slate-500">Dasar Bonus</p><p class="font-medium text-slate-900">{{ $basisLabels[$rule->bonus_basis] }}</p></div>
            <div><p class="text-xs text-slate-500">Dasar Reward</p><p class="font-medium text-slate-900">{{ $rewardLabels[$rule->reward_basis] }}</p></div>
        </div>
        @endcan
    </div>
    ...
    <div class="overflow-hidden border border-slate-200 bg-white">
        <div class="border-b border-slate-200 px-5 py-3">
            <h3 class="font-jakarta text-sm font-bold text-slate-900">Tiers</h3>
            <p class="mt-0.5 text-xs text-slate-500">Tier dipilih berdasarkan masa kerja (bulan) DAN achievement%. Kedua dimensi harus masuk range.</p>
        </div>

        @can('bonus.marketing-rule.create')
        <form method="POST" action="{{ route('bonus.marketing-rules.tiers.store', $rule) }}" class="border-b border-slate-200 bg-slate-50 p-4">
            @csrf
            <p class="mb-3 text-xs font-semibold text-slate-600">Tambah tier</p>
            <div class="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
                <div>
                    <label class="mb-1 block text-xs font-medium text-slate-700">Masa kerja min (bln)</label>
                    <input type="number" name="minimum_tenure_months" value="{{ old('minimum_tenure_months', 0) }}" required min="0" class="block w-full border-slate-300 text-sm focus:border-[#215aac] focus:ring-[#215aac]">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-medium text-slate-700">Masa kerja maks</label>
                    <input type="number" name="maximum_tenure_months" value="{{ old('maximum_tenure_months') }}" min="0" placeholder="kosong=tak terbatas" class="block w-full border-slate-300 text-sm focus:border-[#215aac] focus:ring-[#215aac]">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-medium text-slate-700">Achievement min (%)</label>
                    <input type="number" name="minimum_achievement_percentage" value="{{ old('minimum_achievement_percentage', 0) }}" required min="0" step="0.01" class="block w-full border-slate-300 text-sm focus:border-[#215aac] focus:ring-[#215aac]">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-medium text-slate-700">Achievement maks</label>
                    <input type="number" name="maximum_achievement_percentage" value="{{ old('maximum_achievement_percentage') }}" min="0" step="0.01" placeholder="kosong=tak terbatas" class="block w-full border-slate-300 text-sm focus:border-[#215aac] focus:ring-[#215aac]">
                </div>
                <div>
        ...
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($rule->tiers->sortBy(['minimum_tenure_months', 'minimum_achievement_percentage']) as $tier)
                    <tr class="hover:bg-slate-50/70">
                        <td class="px-5 py-3 text-slate-700">
                            {{ $tier->minimum_tenure_months }}–{{ $tier->maximum_tenure_months ?? '∞' }} bln
                        </td>
                        <td class="px-5 py-3 text-slate-700">
                            {{ $tier->minimum_achievement_percentage }}%–{{ $tier->maximum_achievement_percentage ? $tier->maximum_achievement_percentage . '%' : '∞' }}
                        </td>
                        ...
                        <td class="px-5 py-3 text-right">
                            @can('bonus.marketing-rule.update')
                                <form method="POST" action="{{ route('bonus.marketing-rules.tiers.destroy', [$rule, $tier]) }}" onsubmit="return confirm('Hapus tier ini?')" class="inline">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-8 text-center text-sm text-slate-400">Belum ada tier. Tambahkan tier di atas.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
