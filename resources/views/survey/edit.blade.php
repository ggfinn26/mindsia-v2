...
<div class="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8"
    x-data="{
        questions: {{ $survey->questions->map(fn ($q) => [
            ...
            'choices' => $q->choices->map(fn ($c) => ['choice_text' => $c->choice_text])->values()->all(),
        ])->values()->toJson() }},
        addQuestion() {
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <a href="{{ route('surveys.show', $survey) }}" class="mb-1 flex items-center gap-1 text-xs font-medium text-slate-500 hover:text-slate-700">
                <span class="material-symbols-outlined text-[16px]">arrow_back</span> {{ $survey->survey_name }}
            </a>
            <h1 class="text-2xl font-bold text-gray-900">Edit Survey</h1>
        </div>
        <a href="{{ route('surveys.show', $survey) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg font-medium border border-gray-300">Batal</a>
    </div>
    ...
    <form action="{{ route('surveys.update', $survey) }}" method="POST">
            ...
            <div class="px-6 py-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="survey_name" value="{{ old('survey_name', $survey->survey_name) }}" required maxlength="255" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('survey_name')
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="survey_description" rows="3" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('survey_description', $survey->survey_description) }}</textarea>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="deadline_at" value="{{ old('deadline_at', $survey->deadline_at?->format('Y-m-d\TH:i')) }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('deadline_at')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        <div class="flex gap-3">
            <button type="submit" class="inline-flex min-h-10 items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-5 rounded-md text-sm">
                <span class="material-symbols-outlined text-[18px]">save</span> Perbarui Survey
            </button>
            <a href="{{ route('surveys.show', $survey) }}" class="inline-flex min-h-10 items-center border border-gray-300 px-4 text-sm font-semibold text-gray-700 hover:bg-gray-50 rounded-md">Batal</a>
        </div>
    </form>
</div>
@endsection
