...
@section('title', 'Edit Assignment Evaluator')
@section('header_title', 'Edit Assignment Evaluator')
...
<div class="max-w-2xl mx-auto space-y-6">
    <div>
        ...
        <h2 class="font-jakarta text-2xl font-bold tracking-tight text-slate-900">Edit Assignment</h2>
    </div>
    ...
    <form method="POST" action="{{ route('kpi.evaluator-assignments.update', $assignment) }}" class="border border-slate-200 bg-white p-6 space-y-4">
        @csrf @method('PUT')
        <div>
            ...
            <select name="evaluator_employee_id" required class="block w-full border-slate-300 text-sm focus:border-[#215aac] focus:ring-[#215aac]">
                @foreach ($employees as $emp)
                    <option value="{{ $emp->id }}" @selected(old('evaluator_employee_id', $assignment->evaluator_employee_id) == $emp->id)>{{ $emp->full_name }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label class="mb-1.5 block text-sm font-medium text-slate-700">Evaluatee</label>
            <select name="evaluatee_employee_id" required class="block w-full border-slate-300 text-sm focus:border-[#215aac] focus:ring-[#215aac]">
                @foreach ($employees as $emp)
                    <option value="{{ $emp->id }}" @selected(old('evaluatee_employee_id', $assignment->evaluatee_employee_id) == $emp->id)>{{ $emp->full_name }}</option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <select name="kpi_template_id" required class="block w-full border-slate-300 text-sm focus:border-[#215aac] focus:ring-[#215aac]">
                @foreach ($templates as $t)
                    <option value="{{ $t->id }}" @selected(old('kpi_template_id', $assignment->kpi_template_id) == $t->id)>{{ $t->template_name }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label class="mb-1.5 block text-sm font-medium text-slate-700">Berlaku mulai</label>
            <input type="date" name="effective_start_date" value="{{ old('effective_start_date', $assignment->effective_start_date?->toDateString()) }}" required class="block w-full border-slate-300 text-sm focus:border-[#215aac] focus:ring-[#215aac]">
        </div>
        <div class="flex items-center gap-2">
            <input type="hidden" name="is_active" value="0">
            <input type="checkbox" name="is_active" value="1" id="is_active" @checked(old('is_active', $assignment->is_active)) class="border-slate-300 text-[#215aac] focus:ring-[#215aac]">
