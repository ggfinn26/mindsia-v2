...
<div class="max-w-lg mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <form method="POST" action="{{ route('holidays.update', $holiday) }}" class="bg-white shadow rounded-lg divide-y divide-gray-100">
        @csrf @method('PUT')
        ...
        <div class="px-6 py-5 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="holiday_start_date" value="{{ old('holiday_start_date', $holiday->holiday_start_date->format('Y-m-d')) }}"
                           class="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="date" name="holiday_end_date" value="{{ old('holiday_end_date', $holiday->holiday_end_date->format('Y-m-d')) }}"
                           class="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
    ...
    <form action="{{ route('holidays.destroy', $holiday) }}" method="POST" class="mt-4 text-center"
          onsubmit="return confirm('Hapus hari libur ini?')">
        @csrf @method('DELETE')
        <button type="submit" class="text-sm font-medium text-red-600 hover:text-red-700">Hapus hari libur ini</button>
    </form>
</div>
@endsection
