...
<div class="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8"
    x-data="{
        actions: {{ json_encode($rule->actions->sortBy('action_order')->map(fn($a) => ['action_type' => $a->action_type, 'action_order' => $a->action_order])->values()) }},
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Edit Aturan: {{ $rule->rule_name }}</h1>
    ...
    <form action="{{ route('attendance-rules.update', $rule->id) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="px-6 py-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="rule_name" value="{{ old('rule_name', $rule->rule_name) }}" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                    </div>
                    <div>
                        ...
                        <select name="attendance_type" required class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                            <option value="work_schedule" {{ old('attendance_type', $rule->attendance_type) === 'work_schedule' ? 'selected' : '' }}>Kerja Harian</option>
                            <option value="session" {{ old('attendance_type', $rule->attendance_type) === 'session' ? 'selected' : '' }}>Sesi/Kelas</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="period_type" required class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                            <option value="daily" {{ old('period_type', $rule->period_type) === 'daily' ? 'selected' : '' }}>Harian</option>
                            <option value="monthly" {{ old('period_type', $rule->period_type) === 'monthly' ? 'selected' : '' }}>Bulanan</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="trigger_type" required class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                            <option value="consecutive_absence" {{ old('trigger_type', $rule->trigger_type) === 'consecutive_absence' ? 'selected' : '' }}>Absen Beruntun</option>
                            <option value="monthly_absence" {{ old('trigger_type', $rule->trigger_type) === 'monthly_absence' ? 'selected' : '' }}>Absen Bulanan</option>
                            <option value="monthly_late_count" {{ old('trigger_type', $rule->trigger_type) === 'monthly_late_count' ? 'selected' : '' }}>Jumlah Keterlambatan</option>
                            <option value="monthly_late_minutes" {{ old('trigger_type', $rule->trigger_type) === 'monthly_late_minutes' ? 'selected' : '' }}>Total Menit Terlambat</option>
                            <option value="daily_late" {{ old('trigger_type', $rule->trigger_type) === 'daily_late' ? 'selected' : '' }}>Terlambat Harian</option>
                        </select>
                    </div>
                        ...
                        <div>
                            ...
                            <select name="trigger_operator" required class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                                @foreach(['=' => '=', '>' => '>', '>=' => '≥', '<' => '<', '<=' => '≤'] as $val => $label)
                                    <option value="{{ $val }}" {{ old('trigger_operator', $rule->trigger_operator) === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Nilai <span class="text-red-500">*</span></label>
                            <input type="number" name="trigger_value" value="{{ old('trigger_value', $rule->trigger_value) }}" min="0" step="0.01" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                        </div>
                    </div>
                    <div>
                        ...
                        <select name="is_active" class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                            <option value="1" {{ old('is_active', $rule->is_active) ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ !old('is_active', $rule->is_active) ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                    </div>
                </div>
            </div>
