...
@section('title', 'Tambah Policy Absensi')

@section('content')
<div class="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8" x-data="{ scope: '{{ old('attendance_scope', 'branch') }}', isExempt: {{ old('is_attendance_exempt', false) ? 'true' : 'false' }} }">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Tambah Policy Absensi</h1>
    ...
    <form action="{{ route('attendance-policies.store') }}" method="POST">
        @csrf
        <div class="bg-white shadow sm:rounded-lg divide-y divide-gray-200">
            ...
            <div class="px-6 py-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="policy_name" value="{{ old('policy_name') }}" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="is_attendance_exempt" value="1" @change="isExempt = $event.target.checked" {{ old('is_attendance_exempt') ? 'checked' : '' }} class="h-4 w-4 text-blue-600 border-gray-300 rounded">
                    ...
                    <div class="md:col-span-2" x-show="isExempt">
                        ...
                        <input type="text" name="exemption_reason" value="{{ old('exemption_reason') }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                    </div>
            ...
            <div class="px-6 py-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-3">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="work_is_required" value="1" {{ old('work_is_required', true) ? 'checked' : '' }} class="h-4 w-4 text-blue-600 border-gray-300 rounded">
                    ...
                    <div>
                        ...
                        <input type="number" name="work_late_tolerance_minutes" value="{{ old('work_late_tolerance_minutes', 0) }}" min="0" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Toleransi Pulang Cepat (menit)</label>
                        <input type="number" name="work_early_leave_tolerance_minutes" value="{{ old('work_early_leave_tolerance_minutes', 0) }}" min="0" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                    </div>
                </div>
            </div>
            ...
            <div class="px-6 py-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-3">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="session_is_required" value="1" {{ old('session_is_required', true) ? 'checked' : '' }} class="h-4 w-4 text-blue-600 border-gray-300 rounded">
                    ...
                    <div>
                        ...
                        <input type="number" name="session_late_tolerance_minutes" value="{{ old('session_late_tolerance_minutes', 0) }}" min="0" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 sm:text-sm">
                    </div>
                </div>
            </div>

            <div class="px-6 py-4 flex justify-end">
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-md">Simpan Policy</button>
            </div>
        </div>
    </form>
</div>
